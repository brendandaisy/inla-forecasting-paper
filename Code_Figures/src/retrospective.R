...
rsv_truth <-  function(na.rm = TRUE) { 
  rsv <-read_csv("../data/2024-09-06_rsvnet_hospitalization.csv")
  ...
  disease <- left_join(rsv, name_key, by = "location")
...
generate_save_weekly_forecast_besag <- function(forecast_dates, disease_df, folder_name) {
  ...
  pred_quantiles <- map(forecast_dates, \(fd) {
    ...
    # Explicitly pass the forecast_date (fd) to fit_inla_model
    fit <- fit_inla_model(fit_df, forecast_date = fd, model, pc_prior_u = c(1, 1), graph = graph)
    
    q <- c(0.01, 0.025, seq(0.05, 0.95, by = 0.05), 0.975, 0.99)
    
    # Sample predictions
    nat_samps <- sample_national(fit_df, fit, fd, nsamp = 5000)
    pred_samples <- sample_count_predictions(fit_df, fit, fd, nsamp = 5000)
    
    # Summarize the quantiles and return the cleaned forecasts
    cleaned_forecasts_quantiles <- summarize_quantiles_flusight(pred_samples, nat_samps, forecast_date = fd, q = q)
    
    list(pred = cleaned_forecasts_quantiles, fit_df = fit_df, fit = fit)
  })
  
  pred_list <- lapply(pred_quantiles, function(x) x$pred)
  
  # Subtract 7 days from the reference_date in each dataframe
  pred_list <- lapply(pred_list, function(df) {
    df$reference_date <- ymd(df$reference_date) - 7
    return(df)
  })
